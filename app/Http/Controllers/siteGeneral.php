...
class siteGeneral extends Controller
{
	...
	public function future_expension_component_edit_view($id)
	{
		check_auth();
		check_power('admin');
		$data = DB::select('select * from codebumble_future_expension where id = ?',[$id]);

		$pageConfigs = ['pageHeader' => false];
		return view('/content/site-settings/edit-future-expension', [
			'pageConfigs' => $pageConfigs,
			'data' => $data[0],
		]);
	}

	public function future_expension_component_edit(Request $r, $id){
		...
		return redirect()->route('future_expension_component_edit_view', ['id' => $id, 'hasher' => Str::random(40), 'time' => time(), 'exist' => 'Site Information Updated !! Your Server may take a soft restart for visible the changes. Take A time if It is Down for a short. Thank You', 'hasher_ip' => Str::random(10),]);

	}

	public function future_expension_photos_view()
	{
		check_auth();
		check_power('admin');
		$data = DB::select('select * from codebumble_future_expension');

		$pageConfigs = ['pageHeader' => false];
		return view('/content/site-settings/future-expension-photos', [
			'pageConfigs' => $pageConfigs,
			'future_expensions' => $data,
		]);
	}

	public function future_expension_photos_add(Request $r)
	{
		check_auth();
		check_power('admin');
